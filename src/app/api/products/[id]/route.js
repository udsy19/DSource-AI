...
export async function PATCH(request, { params }) {
  ...
  if (body.product_name !== undefined)
    updates.product_name = sanitizeString(body.product_name);
  if (body.product_id !== undefined) {
    const pid = Number(body.product_id);
    if (Number.isFinite(pid)) updates.product_id = pid;
  }
  if (body.product_material_depot_variant_handle !== undefined)
    updates.product_material_depot_variant_handle = sanitizeString(
      body.product_material_depot_variant_handle,
    );
  if (body.brand_name !== undefined)
    updates.brand_name = sanitizeString(body.brand_name);
  if (body.category_name !== undefined)
    updates.category_name = sanitizeString(body.category_name);
  if (body.color !== undefined) updates.color = sanitizeString(body.color);
  if (body.color_code !== undefined)
    updates.color_code = sanitizeString(body.color_code);
  if (body.color_family !== undefined)
    updates.color_family = sanitizeString(body.color_family);
  if (body.sub_category !== undefined)
    updates.sub_category = Array.isArray(body.sub_category)
      ? body.sub_category
      : parseArrayField(body.sub_category);
  if (body.series_name !== undefined)
    updates.series_name = sanitizeString(body.series_name);
  if (body.description !== undefined)
    updates.description = sanitizeString(body.description);
  if (body.application !== undefined)
    updates.application = Array.isArray(body.application)
      ? body.application
      : parseArrayField(body.application);
  if (body.thickness !== undefined)
    updates.thickness = sanitizeString(body.thickness);
  if (body.size !== undefined) updates.size = sanitizeString(body.size);
  if (body.tags !== undefined)
    updates.tags = Array.isArray(body.tags)
      ? body.tags
      : parseArrayField(body.tags);
