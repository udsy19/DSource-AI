-- Admin control plane: role, RLS carve-outs, capture tables, storage buckets.

-- 1. Admin role: widen profiles role CHECK + revocation columns
...
-- 2. is_admin() used by RLS (app_metadata role only; not user-spoofable)
...
-- 3. Admin carve-outs on existing owner-scoped tables (permissive, OR-ed)
...
create policy "products_admin_select" on public.scraped_product_list for select using (public.is_admin());
drop policy if exists "products_admin_update" on public.scraped_product_list;
create policy "products_admin_update" on public.scraped_product_list for update using (public.is_admin()) with check (public.is_admin());
drop policy if exists "products_admin_delete" on public.scraped_product_list;
create policy "products_admin_delete" on public.scraped_product_list for delete using (public.is_admin());

-- 4. Capture tables
create table if not exists public.ai_generation_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  model text, prompt text, enhanced_prompt text,
  space_type text, style text, lighting text, color_palette text,
  input_image_path text,
  status text not null default 'success', error_code text,
...
create table if not exists public.ai_analysis_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  model text, input_image_path text,
  is_interior boolean, space_type text, overall_confidence numeric,
  detected_categories jsonb, summary text,
  status text not null default 'success', error_code text,
...
create table if not exists public.generated_designs (
  ...
  storage_path text not null, thumbnail_path text, prompt text, source_image_path text,
  width integer, height integer, is_deleted boolean not null default false,
  created_at timestamptz not null default now()
);
create table if not exists public.room_uploads (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  storage_path text not null, mime_type text, size_bytes integer, source text,
  created_at timestamptz not null default now()
);
create table if not exists public.activity_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users (id) on delete set null,
  event_type text not null, target_type text, target_id text,
...
create table if not exists public.admin_audit (
  ...
  action text not null, target_user_id uuid, target_type text, target_id text,
...
declare t text;
begin
  foreach t in array array['ai_generation_events','ai_analysis_events','generated_designs','room_uploads','activity_events'] loop
...
create policy "admin_audit_admin_insert" on public.admin_audit for insert to authenticated with check (public.is_admin());
drop policy if exists "admin_audit_admin_select" on public.admin_audit;
create policy "admin_audit_admin_select" on public.admin_audit for select to authenticated using (public.is_admin());

-- 5. Storage buckets + policies (path "{user_id}/{filename}")
insert into storage.buckets (id, name, public) values ('room-uploads', 'room-uploads', false) on conflict (id) do nothing;
...
create policy "capture_upload_own" on storage.objects for insert to authenticated
  with check (bucket_id in ('room-uploads', 'generated-designs') and (storage.foldername(name))[1] = auth.uid()::text);
drop policy if exists "capture_read_own_or_admin" on storage.objects;
create policy "capture_read_own_or_admin" on storage.objects for select to authenticated
  using (bucket_id in ('room-uploads', 'generated-designs') and ((storage.foldername(name))[1] = auth.uid()::text or public.is_admin()));
drop policy if exists "capture_admin_delete" on storage.objects;
create policy "capture_admin_delete" on storage.objects for delete to authenticated
  using (bucket_id in ('room-uploads', 'generated-designs') and public.is_admin());
